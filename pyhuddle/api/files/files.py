...
from ..huddleerrors import  HuddleConflictError
...
class Document(Api):
    ...
    def restore(self):
        ...
        """
        documentUrl = self.lazyGet().graveyard[0].selfLink + "/restore"
        response = self._adapter.putRequest(documentUrl, self._baseHeader, "{}")
        self.lazyGet().graveyard.pop()
    ...
    def copyTo(self, folder):
        ...
        """
        copyUrl = self.lazyGet().getLink("copy")

        assert getattr(folder, "lazyGet")
    ...
    def __createLock(self):
        ...
        """
        lockUrl = self.lazyGet().getLink("lock")
    ...
    def download(self):
        ...
        for link in self.lazyGet().jsonObj['links']:
    ...
    def moveTo(self, folder):
        ...
        """
        parent = self.lazyGet().getLinkIndex('parent')
        moveUri = self.lazyGet().getLink("move")

        assert parent != -1
        assert moveUri is not None
        assert hasattr(folder, "lazyGet")
    ...
    def versionHistory(self):
        ...
        """
        url = self.lazyGet().getLink("version-history")
        assert url is not None
    ...
    def getComments(self):
        url = self.lazyGet().getLink("comments")
        assert url is not None
...
class Folder(Api):
    ...
    def getFolders(self):
        ...
        for folder in self.lazyGet().jsonObj['folders']:
    ...
    def getDocuments(self):
        ...
        for document in self.lazyGet().jsonObj['documents']:
    ...
    def createDocument(self, file, description="", index=None):
        ...
        try:
            if index is None:
                url = self.lazyGet().getLink("create-document")
            else:
                url = self.getFolders()[index].lazyGet().getLink("create-document")
    ...
    def createFolder(self, title, description="", index=None):
        ...
        try:
            if index is None:
                url = self.lazyGet().getLink("create-folder")
            else:
    ...
    def updateFolder(self, title=None, description=None, index = None):
        ...
        try:
            if index is None:
                ...
                jsonData = self.getFolders()[index].lazyGet().jsonObj
    ...
    def  deleteFolder(self, index=None):
        ...
        try:
            if index is None:
                url = self.lazyGet().getLink("delete")
            else:
                url = self.getFolders()[index].lazyGet().getLink("delete")
    ...
    def restoreFolder(self, index):
        ...
        try:
            url = self.lazyGet().graveyard[index].selfLink + "/restore"
            response = self._adapter.putRequest(url, self._baseHeader, "{}")

            self.lazyGet().graveyard.pop(index)
    ...
    def moveTo(self, newFolder):
        ...
        assert parent != -1
        assert moveURI is not None
        if not hasattr(newFolder, "lazyGet"): raise TypeError("Your newFolder does not have a lazyGet")
        if not hasattr(newFolder, "selfLink"): raise TypeError("Your newFolder does not have a self link")

        self.lazyGet().jsonObj['links'][parent] = {'href' : newFolder.selfLink, 'rel' : 'parent'}
        header = self._baseHeader.copy()
        header['Content-Type'] = "application/vnd.huddle.data+json"
        response = self._adapter.putRequest(moveURI,header, json.dumps(self.lazyGet().jsonObj))
    ...
    def copyTo(self, newFolder):
        ...
        Raises an TypeError if the newFolder parameter doesn't act like a folder object
        """
        copyUri = self.lazyGet().getLink("copy")

        if not hasattr(newFolder, "lazyGet"): raise TypeError("Your folder object is missing lazyGet")
    ...
    def editPermissions(self, json):
        url = self.lazyGet().getLink("permissions")
...
class WorkSpace(Api):
    ...
    def getWorkspaceEntry(self):
        ...
        api.folder.Folder Object
        """
        documentLibary = self.lazyGet().getLink("documentLibrary")
        assert documentLibary is not None

        return Folder(self._client, documentLibary)
