...
from pyhuddle.api.resource import NonHyperLinkResource, HyperLinkResource
...
class Calendar(Api):
    ...
    def refresh(self):
        return Calendar(self._client, self.selfLink)
    ...
    def getTasks(self):
        ...
        for task in self.metaData.jsonObj['events']:
    ...
    def createTask(self, title, description=None, status="NotStarted", plannedStartDate=None, completedDate=None,
        ...
        if attachments: #we have an attachment
            ...
            for attachment in attachments:
                ...
                uri = attachment.metaData().getLink("download")
                title = attachment.metaData().getTitle()
                description = attachment.metaData().getDescription()
                workSpace = HyperLinkResource(attachment.metaData.jsonObj['workspace'])
